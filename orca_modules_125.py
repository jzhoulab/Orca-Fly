...
class Encoder2stage1(nn.Module):
    def __init__(self):
        """
        The stage1 of the Orca Encoder (125bp)
        """
        super(Encoder2stage1, self).__init__()
        self.lblocks = nn.ModuleList(
            [
                nn.Sequential(
                    nn.MaxPool1d(kernel_size=2, stride=2),
...
class Encoder2stage1_1D(nn.Module):
    def __init__(self, num_1d=None):
        """
        The stage1 of the Orca Encoder (125bp)
        """
        super(Encoder2stage1_1D, self).__init__()
        self.lblocks = nn.ModuleList(
            [
                nn.Sequential(
                    nn.MaxPool1d(kernel_size=2, stride=2),
    ...
    def forward(self, x):
        ...
        if self.num_1d is not None:
            output_1d = self.final_1d(out)
        ...
        if self.num_1d is not None:
            return encodings2[0], encodings2[1], output_1d
        else:
            return encodings2


class Encoder2stage2(nn.Module):
    def __init__(self):
        """
        The second section of the Orca Encoder (4kb resolution to
        128kb resolution)
        """
        super(Encoder2stage2, self).__init__()
        self.lblocks = nn.ModuleList(
            [
                nn.Sequential(
                    nn.MaxPool1d(kernel_size=2, stride=2),
