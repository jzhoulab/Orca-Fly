"""
Stage 1
Input: 250*250=62500 bp
Target: 500*500 125 bp resolution Hi-C matrix
"""
...
from orca_modules_125 import Decoder, Encoder, Encoder2stage1_1D

res = 125
modelstr = "NC14_res125.MP.woCentro.stage1"
sequence_length = 62500
val_size = 100
seed = 3141
n_gpus = 10
val_n_gpus = 4
...
if __name__ == "__main__":
    ...
    target_1d = MultibinGenomicFeatures(
        f"{DATASET}/chromHMM/MBT_20_altcolstate.bed",
        list(range(1, 21)),
        res * 2,
        res * 2,
        (20, mat_size // 2),
    ...
    net = nn.DataParallel(Encoder2stage1_1D(num_1d=20))
    ...
    params = (
        [p for p in net0.parameters() if p.requires_grad]
    ...
    target_1d = torch.zeros((n_gpus, 20, mat_size // 2), dtype=torch.float32)
    while True:
        fillind = 0
        optimizer.zero_grad()
        for ss, tt, tt1d in dataloader:
            if np.isnan(tt.numpy()).mean() > 0.5:
                continue
            sequence[fillind, :] = ss
            target[fillind, :] = tt
            target_1d[fillind, :] = tt1d.squeeze(0)
            ...
            with torch.autocast(enabled=True, device_type="cuda"):
                ...
                encoding1, encoding2, pred_1d = net(encoding0)
                encodings = {
                    1: encoding1,
                    2: encoding2,
                }
                ...
                past_1d_losses.append(loss_1d.detach().cpu().numpy())
            ...
            if i % 500 == 0:
                ...
                print("1d:" + str(np.mean(past_1d_losses[-500:])), flush=True)
                out.write("l1:" + str(np.mean(past_losses[1][-500:])) + "\n")
                out.write("l2:" + str(np.mean(past_losses[2][-500:])) + "\n")
                out.write("1d:" + str(np.mean(past_1d_losses[-500:])) + "\n")
                out.flush()
            ...
            if i % 2000 == 0:
                ...
                for ss, tt in zip(
                    ...
                    (
                        ...
                        encoding2,
                        pred_1d,
                    ) = net(net0(torch.Tensor(sequence.float()).transpose(1, 2).cuda()))
                    ...
                    def eval_step(level, start, coarse_pred=None, plot=False):
                        ...
                        for j in range(pred_np.shape[0]):
                            ...
                            if np.mean(validinds) > 0.3:
                                corrs[level].append(
                                    pearsonr(
                                        pred_np[j, validinds], target_np[j, validinds]
                                    )[0]
                                )
                            else:
                    ...
                    start = 0
                    pred = eval_step(2, start, plot=(count == 2))
                    start = start + 62 * 2
                    pred = eval_step(
                ...
                print(
                    "Cor1 {0}, Cor2 {1}".format(
                        np.nanmean(corrs[1]),
                        np.nanmean(corrs[2]),
                    )
                )
                print(
                    "MSE1 {0}, MSE2 {1}".format(
                        np.nanmean(mses[1]),
                        np.nanmean(mses[2]),
                    )
                )
