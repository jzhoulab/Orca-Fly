...
def adaptive_coarsegrain_gpu(ar, countar, cutoff=5, max_levels=8, min_shape=8):
    ...
    with torch.no_grad():
        ...
        if not np.allclose(Nlog, np.rint(Nlog)):
            ...
            newar = torch.empty(
                (newN, newN), dtype=torch.float
            )  # fitting things in there
            newar[:] = np.nan
        ...
        for i in range(len(countar_cg)):
            ...
            countar_exp = _expand(
                _coarsen(countar_next, operation=torch.min, min_nan=True)
            )
...
def _adaptive_coarsegrain(ar, countar, cutoff=5, max_levels=8, cuda=False):
    """
    ...
    if ar.shape[0] < 9 and ar.shape[1] < 9:
        ...
        return adaptive_coarsegrain_fn(
            ar_padded, countar_padded, cutoff=cutoff, max_levels=max_levels
        )[: ar.shape[0], : ar.shape[1]]

    if ar.shape[0] == ar.shape[1]:
        return adaptive_coarsegrain_fn(
            ar, countar, cutoff=cutoff, max_levels=max_levels
        )
...
class Genomic2DFeatures(Target):
    ...
    def __init__(
        ...
        self.cutoff = cg_cutoff
        self.maxlevels = cg_maxlevels
    ...
    def get_feature_data(self, chrom, start, end, chrom2=None, start2=None, end2=None):
        ...
        if self.cg:
            for c in self.data:
                mat_bal = c.matrix(balance=True).fetch(*query)
                if np.mean(np.isnan(mat_bal)) >= self.filt_tresh:
                    out.append(None)
                    continue

                mat_unbal = c.matrix(balance=False).fetch(*query)
                out.append(
            ...
            for c in self.data:
                ...
                if np.mean(np.isnan(mat_bal)) >= self.filt_tresh:
                    ...
                    out.append(mat_bal.astype(np.float32))
        if len(out) == 1:
            out = out[0]
        else:
...
class Genomic2DFeatures_np(Target):
    """
    Stores one or multple datasets of Hi-C style 2D data in cooler format.
    ...
    def get_feature_data(self, chrom, start, end, chrom2=None, start2=None, end2=None):
        ...
        mat_list = self.data
        index_list = np.load(
            "/archive/bioinformatics/Zhou_lab/shared/rwang/orca/misc/LCL_mega_42B.500.2Mb.preload.index.txt.npy"
        )
        chr_index = (
            str(chrom)
            + "-"
            + str((start // 1000000) * 2000)
            + "-"
            + str((end // 1000000) * 2000)
        )

        try:
            mat = mat_list[np.where(index_list == chr_index)[0][0]]
            n_s = start // 500 - (start // 1000000) * 2000
            out = mat[n_s : n_s + 2000, n_s : n_s + 2000]
            return out
        except:
            print(chr_index)
            return np.full((2000, 2000), np.nan)


# class Genomic2DFeatures_juicer(Target):
#     """
#     Stores one or multple datasets of Hi-C style 2D data in cooler format.
...
#     def __init__(self, input_paths, features, shape, cg=False, cuda=False, res=1000):
#         """
#         Constructs a new `Genomic2DFeatures` object.
#         """
#         if isinstance(input_paths, str) and isinstance(features, str):
...
#         if self.cg:
#             out = []
#             for hic in self.data:
#                 mat1 = hic.getMatrixZoomData(
#                     chrom, chrom2, "observed", "KR", "BP", self.res
#                 ).getRecordsAsMatrix(start, end - self.res, start2, end2 - self.res)
#                 mat2 = hic.getMatrixZoomData(
#                     chrom, chrom2, "observed", "NONE", "BP", self.res
#                 ).getRecordsAsMatrix(start, end - self.res, start2, end2 - self.res)
#                 if mat1.shape[0] == 1:
#                     mat1 = np.zeros(
#                         ((end - start) // self.res, (end - start) // self.res)
#                     )
#                 if mat2.shape[0] == 1:
#                     mat2 = np.zeros(
#                         ((end - start) // self.res, (end - start) // self.res)
#                     )
#                 out.append(
#                     _adaptive_coarsegrain(mat1, mat2, cuda=self.cuda).astype(np.float32)
#                 )
...
#                     chrom, chrom2, "observed", "KR", "BP", self.res
#                 ).getRecordsAsMatrix(start, end, start2, end2)
#                 for hic in self.data
#             ]
#         if len(out) == 1:
#             out = out[0]
    ...
    def get_feature_data(self, chrom, start, end):
        ...
        if len(rows) > 0:
            ...
            if self.mode == "proportion":
                ...
                rows_startb = np.array(rows.Start_b)
                rows_endb = np.array(rows.End_b)
                for i in range(len(rows)):
                    targets[
                        self.feature_index_dict[rows_featurename[i]], rows_index[i]
                    ] += (
                        min(rows_end[i], rows_endb[i])
                        - max(rows_start[i], rows_startb[i])
                    ) / self.bin_size
            else:
                for i in range(len(rows)):
                    targets[
...
class RandomPositionsIntervalsSampler(OnlineSampler):
    """
    ...
    def __init__(
        ...
        """
        super(RandomPositionsIntervalsSampler, self).__init__(
            reference_sequence,
            target,
            features,
            seed=seed,
        ...
        if validation_holdout is None or len(validation_holdout) == 0:
            self.modes = ["train"]

        if test_holdout:
            self.modes.append("test")
            if isinstance(validation_holdout, (list,)) and isinstance(
                test_holdout, (list,)
            ):
            ...
            elif isinstance(validation_holdout, float) and isinstance(
                test_holdout, float
            ):
                self.validation_holdout = validation_holdout
                self.test_holdout = test_holdout
                self._holdout_type = "proportion"
            elif isinstance(validation_holdout, str) and isinstance(test_holdout, str):
                self._holdout_type = "interval"
    ...
    def init(func):
        ...
        def dfunc(self, *args, **kwargs):
            if not self.initialized:
                if self._holdout_type == "interval":
                    self._partition_dataset_interval()
                elif self._holdout_type == "proportion":
                    self._partition_dataset_proportion()
                else:
                    self._partition_dataset_chromosome()
    ...
    def _partition_dataset_interval(self):
        """
    ...
    def _partition_dataset_proportion(self):
        """
    ...
    def _partition_dataset_chromosome(self):
        """
        When holdout sets are created by selecting all samples from a
        specified region (e.g. a chromosome) this method is used to
        divide the data into train/test/validate subsets.
    ...
    def _retrieve_multi(self, chroms, starts, ends, strands=None):
        ...
        start : int
            The start position in the query region.
        end : int
            The end position in the query region.
        strand : str or None, optional
            Default is None. The strand of the region. If None, will
            randomly select a strand.
        ...
        for i, (chrom, start, end) in enumerate(zip(chroms, starts, ends)):
            ...
            for j, (chrom2, start2, end2) in enumerate(zip(chroms, starts, ends)):
                ...
                if retrieved_target is None:
                    print("Target returns None. Sampling again.")
                    return None
                if strand == "-":
                    retrieved_target = np.flip(retrieved_target, -2)
                if strand2 == "-":
    ...
    def sample(self, batch_size=1, mode=None, coordinate_only=False):
        ...
        while n_samples_drawn < batch_size:
            ...
            next_length = self.sequence_length
            start_position = np.random.randint(
                cstart, np.fmax(cstart + 1, cend - next_length)
            )
            start_position -= start_position % self.position_resolution
            ...
            allcoords.append([chroms, starts, ends, strands])
            # print(chroms, starts, ends, strands)
            ...
            if not coordinate_only:
                ...
                if not isinstance(seq, list):
                    ...
                    offset = 0
                    for s in seq:
                        sequences[n_samples_drawn, offset : offset + s.shape[0], :] = s
                        offset = offset + s.shape[0]
...
class RandomPositions1DIntervalsSampler(OnlineSampler):
    """
    Draws samples from pre-specified windows in the reference sequence.
    ...
    def __init__(
        ...
        Constructs a new `IntervalsSampler` object.
        """
        super(RandomPositions1DIntervalsSampler, self).__init__(
            reference_sequence,
            target,
            features,
            seed=seed,
        ...
        while n_samples_drawn < batch_size:
            ...
            if not coordinate_only:
                ...
                if not isinstance(seq, list):
                    ...
                    targets[n_samples_drawn, :] = seq_targets
                    if self.target_1d:
                        target_1ds[n_samples_drawn, :] = seq_target_1ds
                else:
                    offset = 0
                    for s in seq:
                        sequences[n_samples_drawn, offset : offset + s.shape[0], :] = s
                    ...
                    if self.target_1d:
                        ...
                        for t in seq_target_1ds:
                            target_1ds[
                                n_samples_drawn, :, offset : offset + t.shape[1]
                            ] = t
                            offset = offset + t.shape[1]
